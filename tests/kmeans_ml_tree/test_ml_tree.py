...
def test_DNA_entropy():
    ...
    with pytest.raises(util.PartitionFinderError):
        main.call_main(
            "DNA", '--raxml --kmeans entropy --min-subset-size 10 "%s"' % full_path
        )
...
def test_DNA_tiger():
    ...
    with pytest.raises(util.PartitionFinderError):
        main.call_main(
            "DNA", '--raxml --kmeans tiger --min-subset-size 10 "%s"' % full_path
        )
...
def test_morph_entropy():
    ...
    main.call_main(
        "morphology", '--raxml --kmeans entropy --min-subset-size 1 "%s"' % full_path
    )
...
def test_morph_tiger():
    ...
    main.call_main(
        "morphology", '--raxml --kmeans tiger --min-subset-size 1 "%s"' % full_path
    )
