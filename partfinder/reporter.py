...
class TextReporter(object):
    ...
    def write_nexus_summary(self, output, sorted_subsets):
        ...
        for sub in sorted_subsets:
            ...
            subset_number += 1
        output.write(
            "\tcharpartition PartitionFinder = %s;\n" % ", ".join(charpartition)
        )
        output.write("end;\n")
    ...
    def write_mrbayes(self, sch, result, output, sorted_subsets):
        ...
        output.write(
            "Warning: MrBayes only allows a relatively small "
        ...
        for sub in sorted_subsets:
            ...
            if self.cfg.datatype == "DNA":
                ...
                model_text = get_mrbayes_modeltext_protein(
                    sub.best_model, subset_number
                )
            else:
                raise RuntimeError
    ...
    def write_best_scheme(self, result):
        ...
        output.write(
            scheme_header_template % ("model_selection", self.cfg.model_selection)
        )
        ...
        if self.cfg.search.startswith("rcluster"):
            output.write(
                scheme_header_template % ("rcluster-percent", self.cfg.cluster_percent)
            )
            output.write(
                scheme_header_template % ("rcluster-max", self.cfg.cluster_max)
            )

        if self.cfg.search == "kmeans" or self.cfg.search == "krmeans":
            output.write(
                scheme_header_template % ("min-subset-size", self.cfg.min_subset_size)
            )
            output.write(scheme_header_template % ("kmeans based on", self.cfg.kmeans))
            if self.cfg.all_states == True:
                output.write(
                    scheme_header_template
                    % ("--all_states setting used", self.cfg.all_states)
                )
...
def write_citation_text(self):
    ...
    ref_kmeans = (
        "Frandsen, P. B., Calcott, B., Mayer, C., & Lanfear, R. "
    ...
    citation_text.append(
        "If you use this analysis in your published "
        "work, please cite "
    ...
    citation_text.append("\n")
    citation_text.append(
        "For the %s algorithm you used, please cite:\n" % (self.cfg.search)
    )
    ...
    if self.cfg.datatype == "morphology":
        citation_text.append(
            "For the model of morphological evolution you used, please cite:\n"
        )
        citation_text.append("%s\n" % ref_morph)
