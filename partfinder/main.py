...
def parse_args(datatype, cmdargs=None):
    ...
    op.add_option(
        ...
        " Possible regions are 'all' or any of {%s}." % ",".join(get_debug_regions()),
    )
    op.add_option(
        "--all-states",
        action="store_true",
...
def main(name, datatype, passed_args=None):
    ...
    try:
        ...
        log.error(
            """A program that Partitionfinder uses failed. Output
        ...
        log.error(
            """We failed to parse the output of an external program (See
