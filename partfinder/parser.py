...
class Parser(object):
    ...
    def init_grammar(self):
        ...
        alignment_def = Keyword("alignment") + EQUALS + FILE_NAME + SEMICOLON
        alignment_def.setParseAction(self.set_alignment)

        tree_def = Keyword("user_tree_topology") + EQUALS + FILE_NAME + SEMICOLON
    ...
    def define_range(self, text, loc, part):
        ...
        if from_column > to_column:
            raise ParserError(
                text,
                loc,
                "Data block has must have beginning less than end "
    ...
    def check_blocks(self, text, loc, partref):
        ...
        if ov:
            ...
            log.error("%s" % ov)
            log.error(
                "Please change your definitions so each site occurs"
    ...
    def check_block_exists(self, text, loc, partref):
        if partref.name not in self.cfg.user_subsets_by_name:
            raise ParserError(text, loc, "Block %s not defined" % partref.name)
    ...
    def define_scheme(self, text, loc, scheme_def):
        try:
            ...
            raise ParserError(text, loc, "Error in '%s' can be found" % scheme_def.name)
        finally:
    ...
    def parse_configuration(self, s):
        ...
        try:
            ...
            log.error(
                """There was a problem loading your .cfg file, please
                      check and try again"""
            )
            log.error(str(p))
            ...
            if missing:
                ...
                log.info(
                    ...
                    double-checking the semi-colons on other lines"""
                    % p.line
                )
            raise PartitionFinderError
