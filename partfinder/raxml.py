...
def make_ml_topology(alignment_path, datatype, cmdline_extras, scheme, cpus):
    ...
    if datatype == "DNA":
        ...
        command = (
            " -f E -s '%s' -m GTRGAMMA -O -n fastTREE -# 1 -p 123456789 -q '%s' -e 10 "
            % (alignment_path, partition_file)
        )
    elif datatype == "protein":
        log.info("Checking data")
        command = (
            " -f C -s '%s' -m PROTGAMMALG -O -n fastTREE -# 1 -p 123456789 -q '%s' -e 10 "
            % (alignment_path, partition_file)
        )
        log.info("Running full ML tree")
        command = (
            " -f E -s '%s' -m PROTGAMMALG -O -n fastTREE -# 1 -p 123456789 -q '%s' -e 10 "
            % (alignment_path, partition_file)
        )
    ...
    if datatype == "DNA":
        ...
        command = (
            "-f e -s '%s' -t '%s' -m GTRGAMMA -O -n BLTREE -p 123456789 -q '%s' -w '%s' -e 1  "
        ...
        command = (
            "-f e -s '%s' -t '%s' -m PROTGAMMALG -O -n BLTREE -p 123456789 -q '%s' -w '%s' -e 1  "
            % (alignment_path, fast_tree_path, partition_file, os.path.abspath(alndir))
        )
    elif datatype == "morphology":
...
def check_defaults(cmdline_extras):
    ...
    cmdline_extras = "".join(
        [" ", cmdline_extras, accuracy, "-O ", "--silent --no-seq-check "]
    )
...
class Parser(object):
    def __init__(self, cfg):
        ...
        self.root_parser = (
            seconds + lnl + alpha + tree_size + lg4x + OneOrMore(rate_block)
        )
