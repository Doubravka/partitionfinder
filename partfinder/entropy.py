...
def sitewise_entropies(alignment):
    if the_config.datatype == "DNA":
        ...
        dna_states = "ACGT"
        dna_list = [
            np.sum(alignment.data == ord(nuc), axis=0) for nuc in list(dna_states)
        ]
        ...
        amino_list = [
            np.sum(alignment.data == ord(aa), axis=0) for aa in list(aa_states)
        ]
...
def get_replacement_sites(column_entropy, columns):
    ...
    if len(nonzeros) == 0:
        log.warning(
            "The entropy of every site in your alignment is zero, cannot reassign entropies"
        )
        return column_entropy
